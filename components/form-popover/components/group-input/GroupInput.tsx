...
import { CalendarFormFields, CalendarGroup } from '@/types';
...
interface GroupInputProps {
  ...
  form: UseFormReturnType<
    FormPopoverValues,
    (values: FormPopoverValues) => FormPopoverReturnValues
  >;
}
      ...
      {fields.multiGroup ? (
        <GroupMultiSelect
          ...
          handleItemSelect={(item: string) =>
            form.setFieldValue(
              'groups',
              form.values.groups.includes(item)
                ? form.values.groups.filter((i) => i !== item)
                : [...form.values.groups, item]
            )
          }
