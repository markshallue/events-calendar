...
import { arrangeWeekEvents, filterByDate, filterByWeek } from '~/utils';
...
export function TimeViewHeader<T>({
  ...
  const totalEventsOnThisDate =
    orderedEvents.length > 0
      ? Math.max(...orderedEvents.map((x: OrderedCalendarEvent) => x.order + 1))
      : 0;
  ...
  return (
    <div
      className="events-calendar-time-view-header"
    ...
    >
      {weekDatesArray.map((dayRecord, dayIndex) => {
        ...
        return (
          <div
            className="events-calendar-time-view-header-cell"
            data-border={withHeaderBorder}
            key={dayIndex}
          >
            {view === 'week' && (
              <div
