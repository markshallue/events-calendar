...
import { HoursColumn, TimeBackground, TimeIndicator, TimeViewHeader } from './components';
import { useSetInitialScroll } from './hooks';
import { getEventsByDayMap, getWeekDates } from './utils';
...
export function TimeView<T>({
  ...
  const dayMap = useMemo(
    () => getEventsByDayMap<T>(weekDatesArray, timeEvents),
    [timeEvents, weekDatesArray]
  );
        ...
        <div className="events-calendar-time-view-grid-wrapper">
          ...
          >
            ...
            {weekDatesArray.map(({ date }, dayIndex) => {
              ...
              if (showPlaceholder)
                orderedEvents.push(state.placeholderEvent as OrderedCalendarEvent<T>);
