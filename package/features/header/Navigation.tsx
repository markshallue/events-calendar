...
export function Navigation({ activeDate, setActiveDate, view }: NavigationProps) {
  ...
  const handleNext = (increment: CalendarView) =>
    setActiveDate((currDate) => currDate.add(1, increment));
  const handlePrev = (increment: CalendarView) =>
    setActiveDate((currDate) => currDate.subtract(1, increment));
