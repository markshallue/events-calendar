    ...
    <>
      ...
      <PageWrapper withTableOfContents>
        ...
        <Alert variant="light" color="blue" title="Important!" icon={<IconInfoCircle />}>
          The calendar expands to fill its parent container. Always ensure that the parent has an
          explicit height.
        </Alert>
        ...
        <Text>
          To add colors to events, assign each event to one or more groups. If an event is assigned
          to multiple groups, the event will be shown with multiple colors.
        </Text>
        ...
        <Text>
          The calendar will attempt to read the time from the <Code>start</Code> and{' '}
          <Code>end</Code> properties. This can be overridden using the <Code>startTime</Code>{' '}
          property. If no <Code>end</Code> or <Code>endTime</Code> is provided, the event will
          default to a one-hour event.
        </Text>
        <Text>
          The following calendar demonstrates how different permutations of <Code>isAllDay</Code>,{' '}
          <Code>start</Code>, <Code>end</Code>, <Code>startTime</Code> and <Code>endTime</Code> are
          parsed into valid event objects.
        </Text>
        <CodeBlock
          withExpandButton
          tabs={[
