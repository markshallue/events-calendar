    ...
    <>
      ...
      <PageWrapper withTableOfContents gap="4rem">
        <Stack>
          <ContentHeading
            title="Basic"
            subtitle="To create a simple calendar, just pass in an events array."
          />
          <CodeBlock
            withExpandButton
            tabs={[
        ...
        <Stack>
          <ContentHeading
            ...
            subtitle={
              <Text>
                Set the <Code>isFetching</Code> prop to <Code>true</Code> to activate the loading
                overlay.
              </Text>
            }
          />
          <CodeBlock
            withExpandButton
            tabs={[{ fileName: 'AsnycExample.tsx', code: asyncExampleCode }]}
          />
        ...
        <Stack>
          ...
          <CodeBlock
            withExpandButton
            tabs={[{ fileName: 'ResponsiveExample.tsx', code: responsiveExampleCode }]}
          />
          <Stack gap="0">
            <Title order={3}>Result</Title>
        ...
        <Stack>
          <ContentHeading
            ...
            subtitle={
              <>
                <Text>
                  Use the <Code>onEventClick</Code> and <Code>renderPopover</Code> props to bind a
                  popover to events.
                </Text>
                <Text>
                  The renderPopover function provides an object containing the{' '}
                  <Code>clickedEvent</Code>, <Code>newEvent</Code> (for click & drag event creation)
                  and an <Code>onClose</Code> function. Make sure to call the <Code>onClose</Code>{' '}
        ...
        <Stack>
          <ContentHeading
            ...
            subtitle={
              <Text>
                With the <Code>useEventsCalendar</Code> hook, we can manage the calendar state
                externally and create a custom header element.
              </Text>
            }
          />
          <CodeBlock
        ...
        <Stack>
          <ContentHeading
            ...
            subtitle={
              <Text>
                To enable click & drag event creation, set <Code>enableDragCreation</Code> to{' '}
                <Code>true</Code>. You can then use the <Code>onEventCreate</Code> callback to
                handle when the user finishes dragging (for example by opening a popover).
              </Text>
            }
          />
        ...
        <Stack>
          <ContentHeading
            ...
            subtitle={
              <Text>
                Use the <Code>renderContextMenu</Code> prop to bind a context menu to events. Right
                click on an event below to see it in action.
              </Text>
            }
          />
          <CodeBlock
            withExpandButton
            tabs={[{ fileName: 'ContentMenuExample.tsx', code: contextMenuExampleCode }]}
          />
          <CalendarWrapper title="Result">
            <ContextMenuExample />
          </CalendarWrapper>
